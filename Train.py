...
from Model import FTDEA
from DataProcess_train import DataProcess
...
def init_data(args, device):
    ...
    data.trans_index1 = torch.randperm(data.rel1.size(0))[:int(args.rate*data.rel1.size(0))]
    data.trans_index2 = torch.randperm(data.rel2.size(0))[:int(args.rate * data.rel2.size(0))]
    ...
    while len(e1) != len(e1_1) and len(e2) != len(e2_1):
        ...
        while loss >= 0.0015:
            if epoch % args.neg_epoch == 0:
                x1, x2 = get_emb(model, data)
                train_batch, train_batch1, train_batch2 = get_train_batch(x1, x2, data.train_set, data.edge_index1,
                                                                          data.edge_index2, data.trans_index1,
                                                                          data.trans_index2, args.k)
            ...
            epoch = epoch + 1
        x1, x2 = get_emb(model, data)
        e1, e2, pairs = Semi_train(x1, x2, data.edge_index1, data.edge_index2, pair_set, k=1)
        model = FTDEA(data.x1.size(1), args.r_hidden, args.t_hidden).to(device)  # 输入embedding维度大小300
        optimizer = torch.optim.Adam(itertools.chain(model.parameters(), iter([data.x1, data.x2])))
