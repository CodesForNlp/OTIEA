...
class GAT_E_to_R(nn.Module):
    def __init__(self, e_hidden, r_hidden):
        ...
        self.a_h2 = nn.Linear(2*r_hidden, 1, bias=False)
        self.a_t1 = nn.Linear(2*r_hidden, 1, bias=False)
        self.a_t2 = nn.Linear(r_hidden, 1, bias=False)
        self.a_h3 = nn.Linear(2*r_hidden, 1, bias=False)
        ...
        self.s_r2 = torch.nn.Linear(2 * e_hidden+r_hidden, r_hidden)
        # self.s_r3 = torch.nn.Linear(2 * e_hidden + r_hidden, r_hidden)
    ...
    def forward(self, x_e, edge_index, rel, rel_size):
        ...
        x_triple = self.s_r2(torch.cat([x_e[edge_index_h], x_res, x_e[edge_index_t]], dim=1))
        x_r_h = self.w_h(x_e[edge_index_h])
        x_r_t = self.w_t(x_e[edge_index_t])
        e1 = self.a_h1(x_r_h).squeeze() + self.a_h2(torch.cat([x_res2, x_r_t],dim=1)).squeeze()
        e2 = self.a_t1(torch.cat([x_r_h, x_res2],dim=1)).squeeze() + self.a_t2(x_r_t).squeeze()
        e3 = self.a_h3(torch.cat([x_r_h, x_r_t],dim=1)).squeeze() + self.a_t3(x_res2).squeeze()
        ...
        # x_res1=torch.cat([x_r_h1, x_rres, x_r_t1],dim=1)
        x_res=x_res1[rel]+x_triple
        # x_res=self.s_r2(x_res)
        return x_res
...
class GATt_to_R(nn.Module):
    def __init__(self,e_hidden,r_hidden,t_hidden):
        ...
        self.s_r2 = torch.nn.Linear(2 * t_hidden+r_hidden, r_hidden)
        self.s_r1 = torch.nn.Linear(2 * t_hidden, r_hidden)
        self.t_c1 = torch.nn.Linear(e_hidden, t_hidden, bias=True)  # 共享的类型变换层
    ...
    def forward(self,x_e,edge_index,rel,x_res1, rel_size):
        ...
        x_triple = self.s_r2(torch.cat([s_convert_t[0], x_res, s_convert_t[1]], dim=1))
        e1 = self.a_1(x_res1).squeeze() + self.a_5(x_triple).squeeze()
        e3 = self.a_2(x_res1).squeeze() + self.a_4(x_triple).squeeze()
        ...
        x_r_r1 = spmm(torch.cat([rel.view(1, -1), rel_size], dim=0), alpha, rel.max() + 1, edge_index_t.size(0),
                      x_res1)
        x_type = x_r_h1+x_r_r1
        x_res1=x_res1+x_type[rel]+x_triple
        return torch.cat([x_res1, r_in_t], dim=1)
class GATr_to_e(nn.Module):
    def __init__(self, e_hidden, r_hidden, t_hidden):
        ...
        # self.w_r3 = nn.Linear(r_hidden + 2 * t_hidden, e_hidden)
        ...
        # self.a_t1 = nn.Linear(e_hidden, 1, bias=False)
        ...
        # self.a_r4 = nn.Linear(e_hidden, 1, bias=False)
    def forward(self, x_e, x_r, edge_index, rel, rel_size):
        ...
        # # 关系对尾实体的表示
        # e_t = x_e[edge_index_t]
        # e_r = self.w_r3(x_r).squeeze()
        # et = self.a_t1(e_t).squeeze() + self.a_r4(e_r).squeeze()
        # alpha = softmax(F.leaky_relu(et).float(), edge_index_t)
        # x_e_t = F.relu(spmm(torch.cat([edge_index_t.view(1, -1), rel_size], dim=0), alpha, x_e.size(0), x_r.size(0),
        #                     e_r))
        # # x = torch.cat([x_e_h, x_e_t], dim=1)
...
class FTDEA(nn.Module):
    ...
    def forward(self,x,edge_index,rel,edge_index_all, rel_size):
        ...
        # r_f=x_res
        x =self.gatr_to_e(x, r_f, edge_index, rel, rel_size)     #带方向的实体表示
