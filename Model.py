...
class GAT_E_to_R(nn.Module):
    ...
    def forward(self, x_e, edge_index, rel, rel_size):
        ...
        # e1 = self.a_h1(x_r_h).squeeze() + self.a_h3(x_res).squeeze() + self.a_h2(x_r_t).squeeze()
        # e2 = self.a_t1(x_r_t).squeeze() + self.a_t3(x_res).squeeze() + self.a_t2(x_r_t).squeeze()
        ...
        x_r_h1 = spmm(torch.cat([rel.view(1, -1), rel_size], dim=0), alpha, rel.max()+1, edge_index_h.size(0),
                     x_r_h)
        alpha = softmax(F.leaky_relu(e2).float(), rel)
        x_r_t1 = spmm(torch.cat([rel.view(1, -1), rel_size], dim=0), alpha, rel.max()+1, edge_index_t.size(0),
                     x_r_t)
        alpha = softmax(F.leaky_relu(e3).float(), rel)
        x_rres = spmm(torch.cat([rel.view(1, -1), rel_size], dim=0), alpha, rel.max() + 1, edge_index_t.size(0),
                     x_res)
...
class GATt_to_R(nn.Module):
    def __init__(self,e_hidden,r_hidden,t_hidden):
        ...
        # self.highway1 = Highway(r_hidden)
    ...
    def forward(self,x_e,edge_index,rel,x_res1, rel_size):
        ...
        # # e_r = self.w_r(x_res1).squeeze()
        # eh = self.a_7(s_convert_t[0]).squeeze() + self.a_8(x_res1).squeeze()
        # alpha = softmax(F.leaky_relu(eh).float(), rel)
        # x_t_h = F.relu(spmm(torch.cat([rel.view(1, -1), rel_size], dim=0), alpha, rel.size(0), edge_index_h.size(0),
        #                     s_convert_t[0]))
        # # e_r = self.w_r1(x_res1).squeeze()
        # eh = self.a_9(s_convert_t[1]).squeeze() + self.a_10(x_res1).squeeze()
        # alpha = softmax(F.leaky_relu(eh).float(), rel)
        # x_t_t = F.relu(spmm(torch.cat([rel.view(1, -1), rel_size], dim=0), alpha, rel.size(0), edge_index_h.size(0),
        #                     s_convert_t[1]))
        # # x_t=x_t_h+x_t_t+s_convert_t[0]+s_convert_t[1]       #实体在语义下的类型表示

        # x_r_h=self.s_r1(r_in_t1)
        # x_res = self.s_r(r_in_t)
        # # e1 = self.a_1(x_r_h).squeeze() + self.a_2(x_res).squeeze()+self.a_5(x_r_t).squeeze()
        # # e2 = self.a_3(x_r_t).squeeze() + self.a_4(x_res).squeeze()+ self.a_6(x_r_h).squeeze()
        # e1 = self.a_7(x_res+x_res1+x_r_h).squeeze() + self.a_9(x_r_h).squeeze()
        # alpha = softmax(F.leaky_relu(e1).float(), rel)
        # x_r_h = spmm(torch.cat([rel.view(1, -1), rel_size], dim=0), alpha, rel.max()+1, edge_index_h.size(0),
        #              x_r_h)
        # x_type1 = x_r_h
        x_res = self.s_r(r_in_t)
        x_res2=self.s_r1(r_in_t1)
        x_r_h =self.w_h(s_convert_t[0])  #self.w_h(x_e[edge_index_h])
        x_r_t =self.w_t(s_convert_t[1]) #self.w_t(x_e[edge_index_t])
        # x_re=self.a_7(x_res1).squeeze()
        # e1 = self.a_1(x_r_h).squeeze() + x_re+self.a_5(x_r_t+x_r_h+x_res).squeeze()
        # e2 = self.a_3(x_r_t).squeeze() + x_re+ self.a_6(x_r_t+x_r_h+x_res).squeeze()
        # e3 = self.a_2(x_res).squeeze() + x_re+ self.a_4(x_r_t + x_r_h + x_res).squeeze()
        e1 = self.a_1(x_res2).squeeze() + self.a_5(x_res1).squeeze()
        e2 = self.a_3(x_r_t).squeeze() + self.a_6(x_res+x_res1+x_r_h).squeeze()
        e3 = self.a_2(x_res+x_res1).squeeze() + self.a_4(x_res2).squeeze()
        alpha = softmax(F.leaky_relu(e1).float(), rel)
        x_r_h1 = spmm(torch.cat([rel.view(1, -1), rel_size], dim=0), alpha, rel.max()+1, edge_index_h.size(0),
                     x_res2)
        alpha = softmax(F.leaky_relu(e2).float(), rel)
        x_r_t1 = spmm(torch.cat([rel.view(1, -1), rel_size], dim=0), alpha, rel.max()+1, edge_index_t.size(0),
                     x_r_t)
        alpha = softmax(F.leaky_relu(e3).float(), rel)
        x_r_r1 = spmm(torch.cat([rel.view(1, -1), rel_size], dim=0), alpha, rel.max() + 1, edge_index_t.size(0),
                      x_res+x_res1)
        x_type = x_r_h1
        # x_res1=x_res1+x_type[rel]+x_res+x_res2
        x_res1=x_res1+x_type[rel]
        # r_in_t=r_in_t+torch.cat([s_convert_t[0], s_convert_t[1]],dim=1)
...
class GATr_to_e(nn.Module):
    def __init__(self, e_hidden, r_hidden, t_hidden):
        ...
        # self.w_eh=nn.Linear(e_hidden,e_hidden)
        # self.w_et = nn.Linear(e_hidden, e_hidden)
    ...
    def forward(self, x_e, x_r, edge_index, rel, rel_size):
        edge_index_h, edge_index_t = edge_index
        #关系对头实体的表示
        # e_h = self.w_eh(x_e).squeeze()[edge_index_h]
        ...
        x_e_h = F.relu(spmm(torch.cat([edge_index_h.view(1, -1), rel_size], dim=0), alpha, x_e.size(0), x_r.size(0),
                     e_r))
        x_e=x_e+x_e_h

        #关系对尾实体的表示
        # e_t = self.w_et(x_e).squeeze()[edge_index_t]
        ...
        x_e_t = F.relu(spmm(torch.cat([edge_index_t.view(1, -1), rel_size], dim=0), alpha, x_e.size(0), x_r.size(0),
                     e_r))
        ...
        x = x_e + x_e_h
        # x=torch.cat([x_e_h, x_e_t], dim=1)
        # x=x_e+self.highway1(x_e,x_e_h)+self.highway2(x_e,x_e_t)
...
class FTDEA(nn.Module):
    def __init__(self,e_hidden=300,r_hidden=100,t_hidden=100):
        ...
        # self.highway4=Highway(r_hidden)    #融合语义空间和类型空间关系的表示
        # self.e_r=nn.Linear(e_hidden,r_hidden)
        self.gatr_to_e=GATr_to_e(e_hidden, r_hidden, t_hidden)
        # self.highway5 = Highway(e_hidden)
        # self.highway6 = Highway(e_hidden)
        # self.gat1=GAT(e_hidden,e_hidden,e_hidden,[3,5])
    ...
    def forward(self,x,edge_index,rel,edge_index_all, rel_size):
        ...
        # # x = x + self.gcn1(x, edge_index_all)
        # # x = x + self.gcn2(x, edge_index_all)
        # # x = self.highway3(x, self.gcn3(x, edge_index_all))
        # edge_index_h, edge_index_t = edge_index
        # x_res =torch.cat([x[edge_index_t] , x[edge_index_h]],dim=1)
        # # x_res=x[edge_index_t] + x[edge_index_h]
        # #原版本
        # # for i in rel.unique():
        # #     x_r = x[edge_index_t[torch.nonzero(rel == i)]] - x[edge_index_h[torch.nonzero(rel == i)]]
        # #     x_r = torch.mean(x_r.float(), 0)
        # #     x_res[torch.nonzero(rel == i)] = x_r
        # #新版本
        # for i in rel.unique():    #可以考虑只是初始值
        #     x_r_h =torch.mean(x[edge_index_h[torch.nonzero(rel == i)]].float(),0)
        #     x_r_t=torch.mean(x[edge_index_t[torch.nonzero(rel == i)]].float(),0)
        #     x_r =torch.cat([x_r_h, x_r_t],dim=1)
        #     # x_r=x_r_h+x_r_t
        #     x_res[torch.nonzero(rel == i)] = x_r
        #
        # x_res = self.s_r(x_res)
        # x_res=torch.cat([x_res_h, x_res, x_res_t],dim = 1)
        r_f = self.gat_t_to_r(x, edge_index, rel, x_res, rel_size)   #类型增强的关系表示
        # r_f=x_res
        x =self.gatr_to_e(x, r_f, edge_index, rel, rel_size)
        # x_e=x+self.gat1(x,edge_index_all)
        # x_e = torch.cat([x, self.gatr_to_e(x, r_f, edge_index, rel)], dim=1)
